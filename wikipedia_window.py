...
class Wikipedia_show:
	...
	def main(self, command, tabControl):
		def save_image(url_of_image):
			response = request.urlopen(url_of_image)#requests.get(url_of_image)
		...
		try:
			wiki_api_url="https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts|pageimages&exsentences=6&pithumbsize=500&exintro&explaintext&redirects=1&titles="+str(question.replace('_', ' '))#str(title[0])
			print(wiki_api_url)
			read_wiki=request.urlopen(wiki_api_url)#requests.get(wiki_api_url)
