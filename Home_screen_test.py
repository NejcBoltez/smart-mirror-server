...
class Time(Frame):
	...
	def __init__(self, parent):#, tabcontrol):
		...
		current_time=Label(self, font=("Helvetica", 100), fg="white", bg="black")
		current_time.pack(side=TOP,anchor=E)
	...
	def update_clock(self,ti,d):
		current_time.config(text=ti)
		day_label.config(text=d)	

class Weather(Frame):
	def __init__(self, parent):
		Frame.__init__(self, parent, bg="black", padx=0, pady=0)
		WeatherTitle=Label(self, font=("Helvetica", 40), fg="white", bg="black", text="Weather:")
		WeatherTitle.pack()
		WeatherFrame=Frame(self, bg="black")
		WeatherFrame.pack(side=LEFT, fill=BOTH, expand= TRUE, anchor="w")
		WeatherTop=Frame(WeatherFrame, bg="black")
		WeatherTop.pack(side=TOP, fill=BOTH, expand= TRUE, anchor="w")
		WeatherIcon=Label(WeatherTop,width=17, fg="white", bg="black")
		WeatherIcon.pack(side=LEFT, fill=BOTH, expand= TRUE, anchor="w")
		WeatherData=Label(WeatherTop, font=("Helvetica", 15), fg="white", bg="black")
		WeatherData.pack(side=RIGHT, fill=BOTH, expand= TRUE, anchor="w")
		WeatherDataHours=Label(WeatherFrame, font=("Helvetica", 12), fg="white", bg="black")
		WeatherDataHours.pack(side=LEFT, anchor="w")
		getWeather()
		
	def getWeather(self):
		read_weather=Work_with_files.read_weather_main()
		read_weather_h=Work_with_files.read_weather_data()
		update_weather_main(read_weather)
		update_weather_hours(read_weather_h)
	...
	def update_weather_main(self,weather_data):
		...
		try:
			...
			img = Label(WeatherIcon, image=render, width=150, height=100, background="red")
			img.pack(side=LEFT, fill=BOTH, expand= TRUE, anchor="w")
		WeatherData.config(text=weather)
	...
	def update_weather_hours(self,data):
		...
		WeatherDataHours.config(text=w_data)

class News(Frame):
	def __init__(self, parent):	
		Frame.__init__(self, parent, bg="black")
		NewsFrame=Frame(self, background="Black")
		NewsFrame.pack(side=RIGHT)
		NewsTitle=Label(NewsFrame, font=("Helvetica", 30), fg="white", bg="black", text="News:")
		NewsTitle.pack()
		NewsShow=Label(NewsFrame, font=("Helvetica", 12), fg="white", bg="black")
		NewsShow.pack(side=LEFT, fill=BOTH, expand= TRUE, anchor="w")
		getNews()
	...
	def update_news(self,data):
		...
		for i in range(len(data)):
			...
			if (news_source.lower() in ("24ur.com", "rtvslo.si", "siol.net", "racunalniske-novice.com") and count_news<10):
				...
				break
		NewsShow.config(text=select_news)
class Home_screen(Frame):
	...
	def main(self, user, tabs, listening_q):
		...
		tabs=tabs
		isListening=True
		HomeFrame=Frame(tabs, background="black")
		HomeFrame.pack(fill=BOTH, expand=YES)
		home_tab = ttk.Frame(tabs)
		tasks=[]
		tabs.add(HomeFrame, text ="HOME SCREEN")
...
async def get_home(self, user, tabs):
	TopFrame = Frame(HomeFrame, background="black", padx=0, pady=0)
	TopLeftFrame=Frame(TopFrame, background="black")
	TopRightFrame=Frame(TopFrame, background="black")
	
	TopFrame.pack(side=TOP, fill=BOTH)
	TopLeftFrame.pack(side = LEFT, fill=BOTH, padx=50, pady=50)
	TopRightFrame.pack(side = RIGHT, fill=BOTH, padx=50, pady=50)

	Clock=Time(TopLeftFrame)
	Clock.pack(side=TOP)
	Weather=Weather(TopRightFrame)
	Weather.pack(side=TOP)
	News=News(HomeFrame)
	News.pack(side=BOTTOM)
	
	CommandHelpHeader=Label(HomeFrame,font=("Helvetica", 40), fg="white", bg="black", text="HELLO "+user.upper())
	CommandHelpHeader.pack()
	CommandHelp=Label(HomeFrame,font=("Helvetica", 12), fg="white", bg="black",text="First say 'Hey Mirror' then you can try to say:")#Search youtube for\nShow me the forecast\nSearch wikipedia for\nStart the calibration")
	CommandHelp.pack()
